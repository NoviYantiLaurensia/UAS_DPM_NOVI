...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var dataDosen: ArrayList<DataLecturer>
    ...
    private fun refresh() {
        val db = DBDOSEN(this@MainActivity)
        val data = db.tampil()
        repeat(dataDosen.size) {dataDosen.removeFirst()}
        if (data.count > 0){
            while (data.moveToNext()){
                val data = DataLecturer(
                    data.getString(0),
                    data.getString(1),
                    data.getString(2),
                    data.getString(3),
